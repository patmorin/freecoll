...
If $G$ is not a triangulation then we add edges to triangulate it in
...
it becomes a triangulation.  By \thmref{collinear-set}, the property
...
and vertices of $G$ in some order $r_1,\ldots,r_k$.
  We extend the sequence $y_i'$ by inserting additional elements,
  resulting in a
  sequence $y_1<\cdots<y_k$ so that,
  whenever $r_i=v_j$ for some $i\in\{1,\ldots,k\}$ and $j\in\{1,\ldots,|S|\}$,
  then
  $y_i = y_j'$.
  We select
	any triangle $\Delta$ that is compatible with $r_1,\ldots,r_k$ and
	$y_1,\ldots,y_k$ and choose $\epsilon = (1/3)\min\{\,y_{i+1}-y_{i}:
	1\le i \le k-1\,\}$.  \thmref{main} then gives us a \Fary\
