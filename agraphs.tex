...
by the boundary equations, and the intersection between $e_i$ and $Y$ is $(0,y_i)$ by construction. Hence, by~\lemref{order-gives-embedding}, ensuring the existence of a solution $s$ to $A\cdot s=b$ satisfying the ordering constraints is enough to prove~\thmref{a-graph}.
...
constraints play a crucial role because they
...
%t(s_j-s'_j) \le 0\\
\end{equation*}
This inequality holds for $t=0$ and for $t=1$, and each side is linear
in $t$, so the inequality holds for 
every $t\in [0,1]$.
