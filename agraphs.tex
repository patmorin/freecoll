...
we will show that every A-graph $G$ has a \Fary\ embedding with prescribed intersections with $Y$ and a prescribed outer face.
...
\item Let $y_1\le\cdots\le y_m$ be any sequence of numbers where, for
  ...
  and $e_{i+1}$ have a common endpoint in $Y$;
		...
		% \end{compactenum}
\end{compactenum}
Then $G$ has a different \Fary\ embedding in which the intersection
...
The convention in Condition~2 about $e_1$ and $e_m$ being boundary edges is only for
...
Next, we assume w.l.o.g.\ up to a uniform scaling that $\Delta$ and
all the vertices of $G$ are contained in $[-1,1]^2$. Second, we assume
w.l.o.g.\ up to a reflection  with respect to $Y$ that, if the outer
face of $G$ is delimited by a quadrilateral, then the common vertex of $e_1$ and $e_m$ is in $L$, as in Figures~\ref{fig:a-graph} and \ref{fig:outerface-cases}c. In such a case we also assume that the vertex of $\Delta$ incident to both $e_1$ and $e_m$ is in $L$; this is also not a loss of generality, since if the vertex of $\Delta$ incident to both $e_1$ and $e_m$ is in $R$, then $\Delta$ can be reflected with respect to $Y$, obtaining a quadrilateral $\Delta'$ whose vertex incident to both $e_1$ and $e_m$ is in $L$, then a \Fary\ embedding of $G$ can be constructed in which the outer face is $\Delta'$, and finally the \Fary\ embedding can be reflected with respect to $Y$, thus obtaining a \Fary\ embedding of $G$ in which the outer face is $\Delta$. 
...
If the outer face is a triangle, the shape $\Delta$ gives us
 ...
 $e_1$, $e_a$, $e_m$.
 If the triangle is $\alpha\beta\gamma$ with $\gamma\inY$, we
...
\subsection{Setting Proportionality Constraints}
 ...
 Our plan is to morph the given embedding of $G$
into the desired embedding.
We therefore choose the coefficients in the proportionality constraints
so that they are satisfied by the initial embedding.
Specifically, the statement of \thmref{a-graph} assumes that $G$ is a
\Fary\ embedding.  In this embedding, every edge $e$
%intersects $Y$
%in a single point $(0,y_e')$ and
has a slope $s_e'$.  For a particular
vertex $v\in Y$, incident to edges $a_1,\ldots,a_k$ and $b_1,\ldots,b_\ell$
as described above, we use these slopes to set the
...
\begin{proof}
	If $G$ is a plane embedding of a 2-connected graph, then another
	straight-line embedding $G'$ of the same graph $G$ is a \Fary\ embedding provided
...
%faces:
A quadrilateral face $q$
must be non-convex in $G$
by Property 3 of A-graphs, and for each vertex, the two incident edges $q$
go to one side. Thus, Property \thetag{$*$}
 ensures that $q$ is non-crossing in $G'$.
...
\begin{proof}
	By \lemref{order-gives-embedding} every vertex is contained in
	the interior or on the boundary of $\Delta\subset[-1,1]^2$.
	Hence, every $x$-coordinate is in the interval $[-1,1]$.
	If $e_i\prec e_j$, then their common vertex has $x$-coordinate
	$(y_j-y_i)/(s_i-s_j)$.	From $|(y_j-y_i)/(s_i-s_j)|\le 1$ we
	derive $|s_i-s_j|\ge|y_j-y_i| > \epsilon$.
\end{proof}
...
Next we show that, for every $0\le t\le 1$, \lemref{weak-to-strong} holds for the system $A_t\cdot s=b_t$.
...
\begin{proof}
  It suffices to prove that $\Delta_t\subset[-1,1]\times[-\infty,+\infty]$ at which point the proof of \lemref{weak-to-strong} applies.
...
t(s_j-s'_j) \le 0\\
\end{eqnarray*}
The final inequality holds for $t=0$ and for $t=1$, and the left side of 
this inequality is linear with respect to $t$, so the inequality holds for 
every $t\in [0,1]$.
...
\begin{proof}
	...
	\lemref{unique}, this solution is unique, so $\det A_0\neq 0$.
...
(PR3). The case where
case where $v$ lies on the outer face is subsumed by (PR1).
...
%	face of $G$ are called {\em boundary edges}.
	...
	$u,v,w$ such that $Y$ exits an inner face through $uv$, enters the outer face, and then immediately enters
