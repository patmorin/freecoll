...
\begin{rem}\remlabel{uniqueness}
For a quadrilateral outer face, the drawing described by
\thmref{a-graph} is unique when the outer face is fixed. If the outer face
is a triangle and $G$ has $n\ge 4$ vertices then there is a one-parameter
family of drawings.  Both these assertions follow from our proof.
\end{rem}
...
is a quadrilateral, we assume w.l.o.g.\ that the common vertex of
$e_1$ and $e_m$ in $G$ is in $L$, as in Figures~\ref{fig:a-graph} and
\ref{fig:outerface-cases}c, and the vertex of $\Delta$ incident to $e_1$
and $e_m$ is also in $L$; this can be achieved by a reflection of $G$
or $\Delta$ with respect to $Y$.
...
on $Y$ and whether $e_b$ lies in $L$ or $R$.\footnote{This freedom in
the choice of $s_b$ gives the 1-parameter of freedom in the drawing
of $G$ referred to in \remref{uniqueness}.}  Together with the
...
We choose the coefficients in the proportionality constraints
