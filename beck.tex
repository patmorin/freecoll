...
on $C$.  We then apply the results in \secref{quadrangulations} to obtain
...
locations. We begin with the proper case in which the curve $C$
...
$b_1=b(y,\Delta)$.  \thmref{dujmovic-frati} implies that there is some
...
Let $s_1$ and $s_m$ be the slopes of the edges $\beta\gamma$ and
$\alpha\gamma$ of the triangle $\Delta=\alpha\beta\gamma$.  Let $x_1$ and
$x_m$ be the x-coordinates of the $\alpha$ and $\beta$ and let $x_1'$ and
$x_m'$ be the x-coordinates of the corresponding vertices of $\Delta'$.
Then, for each $0\le t\le 1$, we define $\Delta(t)$ as the unique triangle $\alpha_t\beta_t\gamma_t$ such that 
\begin{enumerate}
   \item $\alpha_t\gamma_t$ has slope $(1-t)s_m'+ts_m$ and intersects the y-axis at $y_m$ \enspace. 


Let $s_1$ be the slope of the segment $\beta gamma$ and let $s_m$ be the slope of the segment $\beta\gamma$.  For each $0\le t\le 1$, define $s_1(t)=(1-t)s_1'+ts_1$ and $s_m(t)=(1-t)s_m'+t s_m$.  
For each $0\le t\le 1$, let $\Delta(t)=\alpha_t\beta_t\gamma_t$ be the unique triangle such that 
\begin{compactenum}
    \item $\alpha_t\beta_t$ has slope $s_m(t)$ and crosses the y-axis at $y_m$,
...
\begin{proof}
   ...
   $\lim_{t\uparrow t^*} s(t)$ does not exist.  Define the sets $B=\{i\in
   \{1,\ldots,m\}:\text{$\lim_{t\uparrow t^*} s_i(t)$ exists}\}$ and
   $D=\{1,\ldots,m\}\setminus B$. (The set $B$ corresponds to edges of $Q$
   with
   ...
   \begin{enumerate}
    \item $B$ contains the four edges $e_1$, $e_m$, $e_a$ and $e_b$
      on the outer face.
    \item If $i,k\in B$ and $e_i$ and $e_k$ are incident to a common
      vertex $v$ then $j\in B$ for all edges $e_j$ incident to $v$.
    \item If $s_i \prec s_j \prec s_k$ and $i,k\in B$, 
      then $j\in B$.
   \end{enumerate}
...
\begin{proof}
   ...
   on the number of inner faces of $Q'$.  The base cases occur when $Q'$
...
\begin{thm}\thmlabel{main}
   ...
   \begin{compactenum}
     ...
     \item  $C:[0,1]\to\R^2$ be a admissible Jordan curve for $T$;
...
%   of $T$.
   ...
   triangulation $T'$ that has one more proper crossing edge and on which
   we can apply induction.  In the resulting \Fary\ embedding of $T'$,
   the $z$ and $b$ are on opposite sides of the y-axis and $x$ and $y$
   ...
   Since no edge $Q^*$ is in $C$, the vertices in $r_1,\ldots,r_k$ form
   an independent set in $Q^*$.  Therefore, this splitting operation
   can be done on every vertex of $T$ that is on $C$ to obtain a
   quadrangulation $Q$ in which every edge properly crosses $C$, as well
   ...
   the theorem.  Reinserting the proper crossing edges in the quadrangular
