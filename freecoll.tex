...
TODO: Define near-quadrangulation and chord.

\begin{proof}
   We call an edge $e_i$ an \emph{h-edge} (horizontal-edge) if $i\in
   F$ and a \emph{v-edge} (vertical-edge) otherwise.  The proof is by
   induction on the number of edges of $Q'$. In the base case, when $n=4$,
   $Q$ has four edges and they are all on the outer face. In this case,
   the conclusion of the lemma is implied by Condition~1.

   Suppose $Q'$ contains a separating path $v_1,\ldots,v_k$ consisting
   only of h-edges and such that $v_1\neq v_k$ are both on the outer
   face of $Q'$ and none of $v_2,\ldots,v_{k-1}$ are on the outer
   face of $Q'$. In this case we consider each component $A$ of
   $Q'-\{v_1,\ldots,v_k\}$ and apply induction on the subgraph of $Q'$
   induced by $V(A)\cup\{v_1,\ldots,v_k$.  Thus, we may assume that
   $Q'$ has no such separating path.  In particular, this means $Q'$
   is 2-connected and has no chords.

   If $Q'$ has a vertex $v$ of degree-2 on its outer face then $v$ is
   part of a bounded quadrilateral face $vabc$.  By Condition~2, $ab$
   and $bc$ are h-edges.  Therefore, we can remove $v$ and apply induction
   on $Q'-\{v\}$.  Thus, we can assume that $Q'$ has no degree-2 vertices
   on its outer face.  If $Q'$ has an edge $vw$ on its outer face and
   $vw$ is on the boundary of a quadrilaterial $vwab$ such that $ab$
   is an h-edge, then we can remove $vw$ from $Q'$ and apply induction
   on the resulting graph.

%   Consider the subgraph $Q''$ of $Q'$ obtained by removing the outer
%   face of $Q'$.  Since $Q'$ has no chords and has more than 4 vertices,
%   $Q''$ is connected. Furthermore $Q''$ is 2-connected since, otherwise,
%   there is a vertex $v_2$ (a cut vertex in $Q''$) and two vertices
%   $v_1$ and $v_3$ on the outer face of $Q'$ such that $v_1,v_2,v_3$
%   is a separating path consisting only of h-edges.
   ...
   and $c$ and $d$ the lower and upper \emph{eaves} of $q$.  We claim
   that, if a reflex vertex is incident on two h-edges, then all edges of
   $q$ are h-edges. Indeed, with a slight abuse of notation, we have $bc
   \prec ab\prec da\prec cd$.  Then the reflex vertex is $c$, so $bc$ and
   $cd$ are h-edges and, by Condition~2, $ab$ and $da$ are also h-edges.

   We claim that, for any bounded face of $Q'$ that include a vertex $v$
   on the outer face of $Q'$, the vertex $v$ is  not a reflex vertex
   of $Q'$.  Let $v$ be a vertex on the outer face of $Q'$ and let
   $a_1,\ldots,a_r$ be the edges of $Q'$ incident on $Q$ and ordered in
   the counterclockwise order they appear around $q$ and such that $a_1$
   and $a_r$ are on the outer face of $Q'$.  Without loss of generality,
   we may assume that $v\in C^-$, in which case it is straightforward
   to check that $C$ crosses $a_1,\ldots,a_r$ in this order.

   For each $i\in\{1,\ldots,r-1\}$, let $q_i$ be the bounded face of $Q'$
   having $a_i$ and $a_{i+1}$ on its boundary.  Both edges $a_1$ and $a_r$
   are h-edges, so, by Condition~2 $a_1,\ldots,a_r$ are all h-edges.
   We have also determined that $q_1$ and $q_{r-1}$ each have exactly
   one edge that is a v-edge, otherwise we would have removed $a_1$ or
   $a_r$ and applied induction.  Therefore, $v$ is not a reflex vertex in
   $q_1$ or in $q_r$.  By similar reasoning, we can deduce that, if $v$
   is a reflex vertex in $q_i$ for some $i$, then all edges of $q_i$
   are h-edges.  Suppose, therefore, that there is some minimum $i$ such
   that 
