...
\begin{thm}\thmlabel{quad}
    ...
    \item $\Delta$ be a triangle whose interior intersects the y-axis
     in the open segment with endpoints $(0,y_1)$ and $(0,y_m)$.
    \end{compactitem}
...
\thmref{quad} consistently throughout this subsection.
...
\thmref{quad}.  We do this by describing a system of linear equations that
any straight-line embedding that meets the requirements of \thmref{quad}
...
that satisifies the requirements of \thmref{quad}.
...
desired embedding, so that $e_i$ is supported by the line $\{(x,y):y=s_ix
+ y_i\}$.  Note that, since each vertex of $Q$ has degree at least 2,
the values of $s_1,\ldots,s_m$ completely determine a straight-line
embedding of $Q$.
...
edges of $x$ in $\vec{Q}$ or---if $x$ is on $f$---the two edges of $f$
that are incident on $x$.
...
are fixed by $\Delta$.  We will show that $A\cdot s=b$ has a unique
...
\begin{proof}
   ...
   has a non-crossing embedding in $Q'$.  Therefore, by the result cited
...
$A_1=A(y,h)$ and $b_1=b(y,h)$.  \thmref{dujmovic-frati}
implies that there is some straight-line drawing of $Q$ and some
$y_1'<\cdots< y_m'$ such that, for each $i\in\{1,\ldots,m\}$, $e_i$
intersects the y-axis in exactly one point $(0,y_i')$.  Again, without
loss of generality, we may assume that $\Delta'\subset [-1,1]^2$ and that
...
\begin{proof}
   ...
   In the case where $\det(A_t)=0$, the system may have no solutions or
   multiple solutions.  
   Cramer's rules states that, when $\det(A_t)\neq 0$, the solution
   ...
   the function $f_i(t)=\det(A_t^i)$ is also uniformly continuous in $t$.

   We have already established that $A_0\cdot s=b_0$ has a solution $s=s'$
   that satisfies $\prec$. Therefore, by \lemref{unique}, this solution
   is unique, so $\det(A_0)\neq 0$.

   To show that $A_t\cdot s=b_t$
   has a unique solution and that this solution satisfies $\prec$, 
   there are three possible events we must rule out:
   \begin{enumerate}
   ...
   satisfies $\prec$. By \lemref{order-gives-embedding}, this solution
   yields a \Fary\ embedding of $Q$ that satisfies the requirements of
   the theorem, so the proof will be complete.
   ...
   similar to the one which shows the uniqueness of $s'$.  Letting $s(t)$
   denote the unique solution to $A_t\cdot s=b_t$ for all $0\le t\le t^*$,
   we know that $s(t)$ satisfies the $\epsilon^*$ ordering constraints
   for all $0\le t<t^*$, but $s(t^*)$ does not satisfy $\prec$, implying
   a discontinuity in $s(t)$ at $t=t^*$.
   ...
   $s_i(t)-s_j(t)\ge \epsilon^*$ for all $0\le t< t^*$ but $s_i(t)-s_j(t)
   \le 0$ at $t=t^*$.

   Thus, it remains only to rule out the possibility that
   $\det(A_{t*})=0$ and $A_{t^*}\cdot s=b_{t^*}$ has no solutions because
   $\lim_{t\uparrow t^*} s(t)$ does not exist.  Define the sets $B=\{e_i\in
   \{e_1,\ldots,e_m\}:\text{$\lim_{t\uparrow t^*} s_i(t)$ exists}\}$ and
   $D=\{e_1,\ldots,e_m\}\setminus B$. (The set $B$ corresponds to edges of $Q$
   with
   bounded slope and $D$ corresponds to edges of
   $Q$ with divergent slope; that become vertical as $t\uparrow t^*$.)
   An event of Type~3 occurs if and only if $D$ is non-empty.
   We now show that this is not possible.  The partition
   $(B,D)$ of $\{1,\ldots,m\}$ has he following
   properties:
   \begin{enumerate}
    \item $B$ contains the four edges $e_1$, $e_m$, $e_a$ and $e_b$
      on the outer face.
    \item If $e_i,e_k\in B$ and $e_i$ and $e_k$ are incident to a common
      vertex $v$ then $e_j\in B$ for all edges $e_j$ incident to $v$.
    \item If $i \prec j \prec k$ and $e_i,e_k\in B$, 
      then $e_j\in B$.
   \end{enumerate}
   \lemref{partition}, below, shows that in any such partition, the set
   $D$ is empty, therefore no event of Type~3 occurs.  This 
   completes the proof.
\end{proof}

\begin{lem}\lemlabel{partition}
   Let $Q'$ be a subgraph of $Q$ in which each inner face is a
   quadrilateral. Let $(B,D)$ be a partition of $E(Q')$ such that
   \begin{compactenum}
    \item $B$ contains all edges on the outer face 
      of $Q'$;
    \item if $e_i,e_k\in B$ and $e_i$ and $e_k$ are incident to a common
      vertex $v$ then every edge of $Q'$ incident to $v$ is in $B$; and
    \item if $i \prec j \prec k$ and $e_i,e_k\in B$, 
      then $e_j\in B$.
   \end{compactenum}
   Then $B=E(Q')$ and $D=\emptyset$.
\end{lem}

\begin{proof}
   We call an edge of $Q'$ an \emph{h-edge} (horizontal edge) if it is
   in $B$ and a \emph{v-edge} (vertical edge) otherwise.  Thus, we wish
   to show that all edges of $Q'$ are h-edges.  The proof is by induction
   on the lexicographically-ordered pair $(|F(Q')|,|V(Q')|)$, where $F(Q')$
   is the set of inner faces of $Q'$.  The base cases occur when $Q'$
   has all its edges on the outer face, in which case the conclusion of
   the lemma is implied by Condition~1.
   ...
   Refer to \figref{lemma-y}.  The outer face of $Q'$ is a
...
This immediately yields the following corollary of \thmref{quad}.
...
As is the case with \thmref{quad2} there is an annoying case distinction
...
%   at least 2 crossing edges.  
   ...
   each crossing edge $r_i$ incident on $x$ or $y$ will cross
...
%   of $T$.
   ...
   case, $xy$ is incident on two triangles $xyz$ and $yxb$ with $z\in
   ...
   must be incident on crossing faces $xza$ and $zyc$, respectively.
   ...
   Thus, every unmarked edge of $T$ is incident on two crossing
   ...
   as an independent set $S$ of split edges in $Q$.  \thmref{quad2} then
