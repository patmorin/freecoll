...
%   assume that $s_a < s_1 < s_m < s_b$.  
   ...
   for any $\epsilon<\min\{|y_i-y_j| : 1\le i< j\le m\}$.
   ...
   has x-coordinate $(y_i-y_j)(s_j-s_i)$ outside the interval $[-1,1]$.
   ...
   implies that there is some straight-line drawing of $Q$ in which
   each edge $e_i$ of $Q$ intersects the y-axis in exactly one point
   $(0,y_i')$.  (TODO: Update statement of \thmref{X} to specify
   ordering.)  Again, without loss of generality, may assume that
   $\Delta'\subset [-1,1]^2$ and that $\Delta'$ has two vertices on the
   left of the y-axis and one vertex on the right.
   ...
   $t^*$ converges to a solution $\lim_{t\rightarrow t^{*-}} s(t)$ for
   $t=t^*$ that satisfies the ordering constraint.  We have already argued
   that any solution that satisfies the ordering constraint is unique.

   Thus, it remains only to rule out the possibility that
   $\det(A_{t*})=0$ and this corresponds to a situation in which the
   system $A_{t^*}s=b_{t^*}$ has no solutions.
