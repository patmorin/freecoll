...
\begin{lem}\lemlabel{quad}
    ...
    \item $C:[0,1]\to\R^2$ be a simple curve
     ...
     and with the property that each edge of $Q$ properly intersects $C$.
...
In our proof, we will obtain graphs that are not quite quadrangulations
but that can be made into quadrangulations that satisify the requirements
of \lemref{quad}. However, we require a slight strengthening of
\lemref{quad} that allows us to contract certain edges so that the
resulting vertex can be placed on the y-axis.

Let $Q$ be a quadrangulation satisfying the preconditions of
\lemref{quad}. Then we say an edge $xy$ is a \emph{split edge} of $Q$
if there is a Jordan curve $J$ with $x$ and $y$ in its interior, that intersects $C$ in exactly two points, that
does not intersect any open edge of $Q$, and that contains all neighbours
of $x$ and $y$. See \figref{split-edge}.  
\begin{figure}
   \centering{\includegraphics{figs/split-edge}}
   \caption{A split edge $xy$ in a quadrangulation.}
   \figlabel{split-edge}
\end{figure}
The following theorem essentially says that we can contract indepent set of split edges in $Q$ to place the resulting vertices at prescribed locations on the y-axis.
%
%
%
...
%    \item $S$ be an independent set of split edges of $Q$;
%    \item $\Delta$ be a triangle whose intersection with the y-axis
%     is the segment $p_1p_m$ with endpoints $p_1=(0,y_1)$ and $p_m=(0,y_m)$.
%    \end{compactitem}
%    Then $Q$ has a straight-line
%    drawing in which 
%    \begin{compactitem}
%       \item if $e_i$ is incident on a split edge $d;_
%    \end{compitem}
%
...
%   of $T$.
   ...
   (TODO: Draw figure showing how this works even when $v$ is on the outer
    face.)
   \begin{figure}
      \centering{\includegraphics{figs/split}}
   ...
   After performing this splitting operation on every non-outer-face
   vertex $v$ of $Q$ incident on $C$, we obtain a quadrangulation $Q$
   in which every edge properly crosses $C$.
