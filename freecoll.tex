...
\thmref{quad} consistently throughout this section.
Without loss of generality, we assume that  $\Delta$ %=\alpha\beta\gamma$ 
 has two
vertices $\alpha$
and $\beta$
...
of the outer face $f$ extends uniquely to the rest of the $Q$ so that
 the requirements of
\thmref{quad} are fulfilled.  We do this by describing a system of linear equations that
...
% that are incident to $x$.
% GR: I DON'T UNDERSTAND THIS CALCULATION. WHERE ARE THE FOUR EXTRA EQUATIONS?
...
$i,j\in\{1,\ldots,m\}$ such that $i\prec j$, $s_j-s_i\ge \epsilon$.
Observe that any solution $s$ that satisfies the $\epsilon$-strong
ordering constraints also satisifies the ordering constraints. The following lemma shows
...
\begin{proof}
   ...
   We claim that, if the reflex vertex $c$ is incident to two h-edges,
...
%   of $Q''$.
%
%   If we define the edges of $Q''$ incident to $x$ and $y$ to be h-edges
...
%   $a_1,\ldots,a_r$ be the edges of $Q'$ incident to $Q$ ordered in 
...
%   $vcdy$ of $Q'$ incident to these two edges.  Since $Q'$ contains no
