...
\thmref{dujmovic-frati} implies that we can, without loss of
...
Some addition properties of $G$ following from \defref{a-graph}:
...
Since the rest of this section is a proof of \thmref{a-graph},
we will use the notations $G$, $f$, $e_1,\ldots,e_m$, $y_1,\ldots,y_m$,
and $\Delta$ that appear in the statement of \thmref{a-graph} consistently
...
$e_{i},\ldots,e_{i+d}$.  Refer to \figref{ab}.
...
and $b_1,\ldots,b_l\in Y\cup R$, ordered from bottom to top as in \figref{ab}.
...
%
Therefore, before proving \lemref{partition-extended}, let us see why Proprety~1 in \propref{set-H} implies Assumption 1 of the lemma.  is true: (DOES THIS ARGUMENT BELONG HERE?)
...
%   at least 2 crossing edges.  
   ...
   sufficiently simple structure that it can be handled by \thmref{a-graph}.
...
%   of $T$.
   ...
   as an independent set $S$ of split edges in $Q$.  \thmref{a-graph} then
