...
%   and, without loss of generality, we may
%   assume that $s_a < s_1 < s_m < s_b$.  

   \paragraph{The linear system $As=b$:}
   ...
   A result of Felsner states that $Q$ has an orientation of
   its edges so that every vertex except those on the outer face have
   in-degree 2 and vertices of the outer face have in-degree 1.
   ...
   We claim that any solution $s$ to $As=b$ that satisfies the
   ordering constraints yields a straight-line drawing of $Q$.  It is
   straightforward to check that, if $s$ satisifies the ordering
   constraints then, for any face $g$ of $Q$, the edges of $Q$ in the
   embedding defined by $s$ are the edges of a simple quadrilateral
   whose orientation matches that of $Q$......
   ...
   \paragraph{Uniqueness of solutions satisfying the ordering constraint:}
   ...
   statement of \thmref{X} to specify ordering.)  Again, without loss
   ...
   \begin{enumerate}
    ...
    \item If $s_i < s_j$ is an ordering constraint and $i\in P$,
      then $j\in P$.
    \item If $s_i < s_j < s_k$ are ordering constraints and $i,k\in F$, 
      then $j\in F$.
   \end{enumerate}
   ...
   is defined and continuous.  This implies that $\lim_{t\uparrow t^*}
   d_i(t)$ exists and is equal to 0, which implies that $s^*$ is indeed
   a solution to $A_{t^*}s^*=b_{t^*}$.  This concludes the proof.
\end{proof}
...
satisify the requirements of \lemref{quad}. However, we require a slight
strengthening of \lemref{quad} that allows us to contract certain edges
