...
\[
     \epsilon_1 = \min_{0\le t\le 1}\{s_a(t)-s_1(t), s_1(t)-s_m(t), s_m(t)-s_b(t)\}
\]
...
\]
Let 
\[    \epsilon_2=\min_{0\le t\le 1}
      \min\{\min\{y_j(t)-y_i(t): 1\le i< j\le m\}
\]
...
\begin{proof}
   ...
   be marked or unmarked).  We say that points in $L$ are \emph{to the
...
%   at least 2 crossing edges.  
   ...
   We begin by describing reductions that allow use to apply the inductive
...
%   of $T$.
   ...
   that if some unmarked edge $xy$ is not contractible then one of
   $xy$'s incident faces, $yxb$ is crossing.  Suppose, for the
   ...
   consisting of four proper crossing edges.  Let $Q^*$ denote
