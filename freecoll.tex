...
A \emph{curve} $C:[0,1]\to\R^2$ is a continuous function.  The points
$C(0)$ and $C(1)$ are called the \emph{endpoints} of $C$.  A curve $C$
is \emph{simple} if $C(s)\neq C(t)$ for any $0\le s<t< 1$. A curve $C$
is \emph{closed} if $C(0)=C(1)$.  A Jordan curve $C:[0,1]\to\R^2$ is
a simple closed curve.  When we talk about the order of the points on
a simple curve $C$ we mean the partial order over $\R^2$, where $C(a)$
precedes $C(b)$ if and only if $0\le a<b$
We will often fail to distinguish between a curve $C$ and it's image
$\{C(t):0\le t\le 1\}$.  In such cases we may qualify the curve as
\emph{open} in which case we are referring ot the set $\{C(t):0< t< 1\}$.

All graphs $G$ considered in this paper are finite, simple, and
undirected.   We use $V(G)$ and $E(G)$ to denote the vertex set and
edge set of $G$, respectively.  A \emph{cutset} of $G$ is a set of
vertices whose removal disconnects $G$.  A \emph{separating cycle}
is a cutset whose vertices form a cycle.  A \emph{separating triangle}
is a separating cycle of length 3.
...
\begin{proof}
  ...
  For all sufficiently small $\epsilon >0$, \thmref{quad} ensures
  ...
  \begin{enumerate}
    ...
    \epsilon/\deg(x) > \epsilon/n$.  However, in this case, \corref{split-edges} ensures that the x-coordinate of $x$ is at
  ...
  finish the proof of \lemref{uniqueness} shows that, for each
...
By now we have developed the main tool used to prove that every collinear
set is free.  
...
\begin{thm}\thmlabel{main}
   ...
   \begin{compactenum}
     ...
     \item  $C:[0,1]\to\R^2$ be a nice clean Jordan for $T$;
